...
///     -- CP debit, stat reset, title/halo change, ornament toggle, bonus-item consumption -- to mirror it
///     onto the ACTOR'S OWN <see cref="World.PlayerRuntimeState" />, same "pre-decided result, tick just
///     applies it" contract as <see cref="Inventory.InventoryZoneCommand" />/<see cref="Skills.SkillZoneCommand" />.
///     Every field is nullable/optional and independently applied -- null means "this tSort did not touch
///     this field," not "reset to zero" (see <see cref="World.Zone.ApplyTribeProgressCommand" />).
/// </summary>
/// <param name="CharacterId">
///     Which player this command applies to -- a no-op if they already left the zone. USUALLY the
///     requesting character themselves, but <see cref="TribeRole" /> is the one field this ALSO carries for
///     a DIFFERENT character (TRIBE_WORK tSort 2/3's sub-master target) -- same cross-character-mirror
///     posture as <see cref="Guilds.GuildMembershipZoneCommand" />, reusing this type rather than adding a
///     whole second one for a single field.
/// </param>
...
///     New <c>ReturnTribeRole</c> encoding (0 member, 1 master, 2 sub-master -- <see cref="World.PlayerRuntimeState.TribeRole" />'s
///     own remarks) -- TRIBE_WORK tSort 2 (appoint, -&gt; 2) / tSort 3 (remove, -&gt; 0) mirror this onto the
///     SUB-MASTER TARGET, not the requesting Force Leader, so that character's own next TRIBE_WORK action
///     (tSort 4/16/17/18's role gates) sees the change without needing to re-log.
/// </param>
...
///     Ground-item drop(s) to spawn at the character's CURRENT (tick-thread-fresh) position when this
///     command is applied -- tSort 4/8/16/17/18's <c>ProcessForDropItem</c> calls (doc 10 §2). Spawning a
///     ground item mutates <see cref="World.Zone" />'s own tick-owned <c>_groundItems</c>/<c>_grid</c>-adjacent
///     state (<see cref="World.Zone.SpawnGroundItem" /> is <c>internal</c> precisely because it is NOT
///     thread-safe to call from a request thread), so this is the one avenue this batch's handlers have to
///     request a drop -- always empty/default (never a real failure mode) since Fenrir's ground-item pool
///     has no capacity limit, unlike the legacy's <c>ProcessForDropItem</c> which COULD fail; that failure
///     path is therefore unreachable here (documented, not fabricated).
/// </param>
