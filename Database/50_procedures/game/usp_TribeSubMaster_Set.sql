...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM game.TribeSubMasters WHERE TribeId = @TribeId AND SlotIndex = @SlotIndex)
        THROW 50310, N'Tribe sub-master slot is already occupied.', 1;

    IF EXISTS (SELECT 1 FROM game.TribeSubMasters WHERE TribeId = @TribeId AND CharacterId = @CharacterId)
