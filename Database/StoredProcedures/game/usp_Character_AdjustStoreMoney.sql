...
-- the reverse). Both columns live on the same game.Characters row, so a single guarded UPDATE is already
-- atomic -- no explicit transaction needed, matching usp_Character_AdjustMoney's own shape.
