...
public class CzFishingStateSendTests
{
    ...
    {
        Assert.Equal(12, CzFishingStateSend.PayloadSize);
        Assert.Equal(Opcodes.Zone.Incoming.FishingStateSend, CzFishingStateSend.Opcode);
    }

    [Fact]
    public void TryRead_DecodesFieldsFromManuallyEncodedBuffer()
    {
        Span<byte> buffer = stackalloc byte[CzFishingStateSend.PayloadSize];
        ...
        var ok = CzFishingStateSend.TryRead(buffer, out var packet);
