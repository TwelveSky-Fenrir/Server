...
public sealed class SessionTicketRepositoryTests : IDisposable
{
    ...
    public async Task CreateAsync_ThenConsumeAsync_ReturnsTheStoredCharacterShardAndSessionToken()
    {
        ...
        await _repository.CreateAsync(accountId, 42, 3, 15, sessionToken, CancellationToken.None);
    ...
    public async Task ConsumeAsync_CalledASecondTimeForTheSameAccount_ReturnsNull()
    {
        ...
        await _repository.CreateAsync(accountId, 7, 1, 15, Guid.NewGuid(), CancellationToken.None);
    ...
    public async Task ConsumeAsync_AfterTheTtlHasElapsed_ReturnsNull()
    {
        ...
        await _repository.CreateAsync(accountId, 9, 2, 1, Guid.NewGuid(), CancellationToken.None);
    ...
    public async Task CreateAsync_CalledTwiceWithoutConsuming_TheSecondCallSupersedesTheFirst()
    {
        ...
        await _repository.CreateAsync(accountId, 10, 1, 15, Guid.NewGuid(), CancellationToken.None);
        await _repository.CreateAsync(accountId, 20, 2, 15, secondToken, CancellationToken.None);
