...
public class CzTempRegisterSendTests
{
    ...
    {
        Assert.Equal(263, ZoneHandshakeRequest.PayloadSize);
        Assert.Equal(Opcodes.Zone.Incoming.ZoneHandshake, ZoneHandshakeRequest.Opcode);
    }

    [Fact]
    public void TryRead_DecodesFieldsFromManuallyEncodedBuffer()
    {
        Span<byte> buffer = new byte[ZoneHandshakeRequest.PayloadSize];
        ...
        var ok = ZoneHandshakeRequest.TryRead(buffer, out var packet);
