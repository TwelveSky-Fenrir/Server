...
BEGIN
    ...
    DECLARE @RolledOver BIT = 0;

    BEGIN TRANSACTION;

    IF NOT EXISTS (SELECT 1 FROM game.HeroRankingRolloverState WITH (UPDLOCK, HOLDLOCK) WHERE Id = 1)
        INSERT INTO game.HeroRankingRolloverState (Id) VALUES (1);

    DECLARE @LastRolloverAtUtc DATETIME2(3);
    SELECT @LastRolloverAtUtc = LastRolloverAtUtc
    FROM game.HeroRankingRolloverState WITH (UPDLOCK, HOLDLOCK)
    WHERE Id = 1;

    IF DATEDIFF(DAY, @LastRolloverAtUtc, SYSUTCDATETIME()) >= 7
    BEGIN
        DELETE FROM game.HeroRankings WHERE PeriodKind = 1;
        ...
        INSERT INTO game.HeroRankings (CharacterId, PeriodKind, Points, TribeId, Level, RewardClaimed, Description,
                                        RecordedAtUtc)
        SELECT CharacterId, 1, Points, TribeId, Level, 0, NULL, SYSUTCDATETIME()
        FROM RankedCurrent
        WHERE Rn <= 10;

        DELETE FROM game.HeroRankings WHERE PeriodKind = 0;

        UPDATE game.HeroRankingRolloverState SET LastRolloverAtUtc = SYSUTCDATETIME() WHERE Id = 1;
