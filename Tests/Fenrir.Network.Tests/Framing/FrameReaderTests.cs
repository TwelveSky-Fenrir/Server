...
public class FrameReaderTests
{
    ...
    public void TryReadFrame_SingleCompleteFrame_CzHeartbeatSend_DecodesOpcodeAndPayload()
    {
        ...
        var decoded = FrameReader.TryReadFrame(ref buffer, Registry, Server, out var frame);
        ...
        FenrirServer server)
    {
        ...
        var exception = Assert.Throws<ProtocolViolationException>(() =>
            FrameReader.TryReadFrame(ref buffer, ProviderFor(server), server, out _));
    ...
    public void TryReadFrame_BufferShorterThanHeader_ReturnsFalse_NeverThrows_NeverConsumes(int length)
    {
        ...
        var decoded = FrameReader.TryReadFrame(ref buffer, Registry, Server, out _);
    ...
    public void TryReadFrame_CzTempRegisterSend_FragmentedAtEveryByteBoundary_NeverCorruptsNeverMisfires()
    {
        ...
        for (var cut = 1; cut < frameSize; cut++)
        {
            ...
            var decodedIncomplete = FrameReader.TryReadFrame(ref incomplete, Registry, Server, out _);
            ...
            var decodedComplete = FrameReader.TryReadFrame(ref complete, Registry, Server, out var frame);
    ...
    private static IOpcodeFrameSizeProvider ProviderFor(FenrirServer server)
    {
        return server == FenrirServer.Login ? LoginOpcodeRegistry.Provider : ZoneOpcodeRegistry.Provider;
    }
