...
public class ZcBuyCashItemRecvTests
{
    ...
    {
        Assert.Equal(40, ZcBuyCashItemRecv.PayloadSize);
        Assert.Equal(4 + 4 + 4 + 4 + 24, ZcBuyCashItemRecv.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.BuyCashItemRecv, ZcBuyCashItemRecv.Opcode);
    }
    ...
    public void Write_ProducesGoldenBytes()
    {
        ...
        var packet = new ZcBuyCashItemRecv
        {
            Result = 60704,
            CashSize = 900,
            Page = 1,
        ...
        var actual = new byte[ZcBuyCashItemRecv.PayloadSize];
        var written = packet.Write(actual);
        Assert.Equal(ZcBuyCashItemRecv.PayloadSize, written);

        var expected = new byte[ZcBuyCashItemRecv.PayloadSize];
