...
--
-- FailedLoginCount/LockoutUntilUtc: Fenrir-only addition, no legacy analog (see the pincode-second-password
-- security audit's Major finding "No account-scoped (cross-reconnect) brute-force throttle on the mouse
-- PIN"): LoginClientSession.PinFailureCount (Network/Fenrir.Network.Dispatch.Login/Sessions/LoginClientSession.cs)
...
-- tolerate shared-NAT traffic, not to block brute force). These two columns share
-- auth.Accounts.FailedLoginCount/LockoutUntilUtc's exact names and progressive-lockout shape for the
-- primary password (see StoredProcedures/auth/usp_Account_RecordLoginAttempt.sql) rather than a purely
-- in-memory per-session counter, per that finding's own recommendation; usp_AccountPin_RecordAttempt
-- maintains them.
