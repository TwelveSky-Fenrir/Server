...
public class ClGiftInfoSendHandlerTests
{
    ...
    {
        var handler = new ClGiftInfoSendHandler();
        var pipe = new FakeDuplexPipe();
        var session = new LoginClientSession(1, pipe);
        session.MarkAuthenticated(1);
        session.MarkCharSelect();

        handler.Handle(new ClGiftInfoSend(), session);

        Assert.Null(session.DisconnectReason);
        await PacketAssert.AssertSentAsync(pipe, new LcDemandGiftRecv { Result = 0, GiftItem = new int[20] });
    }
}
