...
public class ClWantGiftSendTests
{
    ...
    public void PayloadSize_MatchesContractConstant()
    {
        ...
        Assert.Equal(8, ClWantGiftSend.PayloadSize);
    }

    [Fact]
    public void RoundTrip_PreservesAllFields()
    {
        var buffer = new byte[ClWantGiftSend.PayloadSize];
        ...
        Assert.True(ClWantGiftSend.TryRead(buffer, out var packet));
    ...
    {
        var buffer = new byte[ClWantGiftSend.PayloadSize - 1];

        Assert.False(ClWantGiftSend.TryRead(buffer, out _));
    }
}
