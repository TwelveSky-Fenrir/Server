...
// Per-session, per-opcode-class buckets (§8.5); keyed on opcode since the legacy wire has no sequence number.
public interface ISessionRateLimiter
{
    ...
    // immediate disconnect rather than §8.5's per-class throttle-vs-disconnect distinction.
