using System.Buffers.Binary;
using Fenrir.Contracts.Packets.Login;

namespace Fenrir.Contracts.Tests.Packets.Login;

public class LcChangeAvatarNameRecvTests
{
    [Fact]
    public void PayloadSize_MatchesContractConstant()
    {
        // ExpectedSize=5 (1-byte outbound header) -> 4-byte payload (1 int).
        Assert.Equal(4, LcChangeAvatarNameRecv.PayloadSize);
    }

    [Fact]
    public void RoundTrip_PreservesAllFields()
    {
        // 102 = the legacy "update failure" code — proves the full legacy code range survives the int.
        var packet = new LcChangeAvatarNameRecv { Result = 102 };

        var buffer = new byte[LcChangeAvatarNameRecv.PayloadSize];
        var written = packet.Write(buffer);

        Assert.Equal(LcChangeAvatarNameRecv.PayloadSize, written);
        Assert.Equal(packet.Result, BinaryPrimitives.ReadInt32LittleEndian(buffer));
    }
}
