...
/// <summary>CZ_MAKE_ITEM2_SEND (CLIENT.h:289, 36-byte payload) — same typedef as <see cref="CzMakeItemSend" />.</summary>
public class CzMakeItem2SendTests
{
    [Fact]
    public void PayloadSize_MatchesContract()
    {
        Assert.Equal(36, CzMakeItem2Send.PayloadSize);
        Assert.Equal(Opcodes.Zone.Incoming.MakeItem2Send, CzMakeItem2Send.Opcode);
    }
    ...
    public void TryRead_GoldenBytes_DecodesEveryField()
    {
        ...
        Assert.True(CzMakeItem2Send.TryRead(golden, out var decoded));
    ...
    {
        Assert.False(CzMakeItem2Send.TryRead(new byte[35], out _));
    }

    private static CzMakeItem2Send CreatePopulated()
    {
        return new CzMakeItem2Send
        {
            Sort = 2,
            Page1 = 22,
            Index1 = 33,
    ...
    private static void EncodeGolden(Span<byte> destination, CzMakeItem2Send value)
    {
