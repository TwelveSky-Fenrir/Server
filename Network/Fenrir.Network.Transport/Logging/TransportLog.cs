...
/// <summary>
///     Operational log entries for the two swallowed-by-design fault paths inside <see cref="FenrirTcpListener{TSession}" />
...
internal static partial class TransportLog
{
    ...
    [LoggerMessage(
        ...
        Message = "Accept on {LocalEndPoint} failed with a transient socket error (half-open scan or similar) -- continuing the accept loop")]
    public static partial void AcceptPortScanSwallowed(this ILogger logger, Exception exception, EndPoint? localEndPoint);
    ...
    [LoggerMessage(
        ...
        Message = "Failed to construct a session for an accepted connection on {LocalEndPoint} -- the accepted socket is being disposed without ever running")]
    public static partial void ConnectionConstructionFailed(this ILogger logger, Exception exception, EndPoint? localEndPoint);
    ...
    [LoggerMessage(
        ...
        Level = LogLevel.Warning,
        Message = "Send loop faulted for connection {RemoteEndPoint} -- this connection can no longer send anything until an independent teardown path notices")]
