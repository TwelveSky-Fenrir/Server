...
public class GuildActionHandlerBroadcastTests
{
    ...
    public async Task Notice_Success_BroadcastsGuildInfo_ToOnlineMemberInAnotherZone_ButNotToAnOutsider()
    {
        ...
        var (masterSession, masterPipe, _) = EnterZone(zones, 1, MasterId, "Odin", GuildId, 2);
        ...
        await handler.HandleAsync(new GuildActionRequest { Sort = 5, Data = data }, masterSession,
    ...
    public async Task Agm_Promote_BroadcastsGuildInfo_ToOnlineThirdMember_NotJustActorAndTarget()
    {
        ...
        await handler.HandleAsync(new GuildActionRequest { Sort = 9, Data = data }, masterSession,
            CancellationToken.None);
    ...
    public async Task Title_Success_BroadcastsGuildInfo_ToOnlineMemberInAnotherZone()
    {
        ...
        await handler.HandleAsync(new GuildActionRequest { Sort = 10, Data = data }, masterSession,
            CancellationToken.None);
    ...
    public async Task Buff_Activation_StampsFreshCheckpoint_AndBroadcastsToOnlineMember()
    {
        ...
        await handler.HandleAsync(new GuildActionRequest { Sort = 14, Data = data }, masterSession,
            CancellationToken.None);
