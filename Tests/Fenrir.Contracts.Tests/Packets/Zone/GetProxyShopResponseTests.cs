...
public class ZcGetDeputyPshopRecvTests
{
    ...
    {
        Assert.Equal(832, GetProxyShopResponse.PayloadSize);
        Assert.Equal(4 + 4 + ProxyShopUserInfo.WireSize, GetProxyShopResponse.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.GetProxyShop, GetProxyShopResponse.Opcode);
    }
    ...
    public void Write_RoundTrips_ViaManualOffsetRead()
    {
        ...
        var packet = new GetProxyShopResponse { Result = 0, Sort = 1, ProxyUser = proxyUser };

        Span<byte> buffer = new byte[GetProxyShopResponse.PayloadSize];
        var written = packet.Write(buffer);

        Assert.Equal(GetProxyShopResponse.PayloadSize, written);
    ...
    public void Write_ProducesGoldenBytes()
    {
        ...
        var packet = new GetProxyShopResponse { Result = 2, Sort = -1, ProxyUser = proxyUser };

        var golden = new byte[GetProxyShopResponse.PayloadSize];
