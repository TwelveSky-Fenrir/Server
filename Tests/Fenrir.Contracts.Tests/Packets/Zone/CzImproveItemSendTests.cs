...
///     padding — same typedef as <see cref="CzAddItemSend" />/<see cref="CzHighItemSend" />/<see cref="CzLowItemSend" />.
/// </summary>
public class CzImproveItemSendTests
{
    ...
    {
        Assert.Equal(20, CzImproveItemSend.PayloadSize);
        Assert.Equal(Opcodes.Zone.Incoming.ImproveItemSend, CzImproveItemSend.Opcode);
    }
    ...
    public void TryRead_GoldenBytes_DecodesEveryField()
    {
        ...
        Assert.True(CzImproveItemSend.TryRead(golden, out var decoded));
    ...
    {
        Assert.False(CzImproveItemSend.TryRead(new byte[19], out _));
    }
}
