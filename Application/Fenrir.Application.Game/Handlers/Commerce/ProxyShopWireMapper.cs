...
/// <summary>Shared PROXY_SHOP_USER_INFO builder for <see cref="GetProxyShopHandler" />/<see cref="UpdateProxyShopHandler" /> -- maps game.OfflineShops/OfflineShopItems rows onto the 25-slot wire shape (unlisted slots are zero).</summary>
        ...
        IReadOnlyList<OfflineShopItemRowDto> items)
    {
        ...
        foreach (var item in items)
        {
            ...
            wireItems[item.SlotIndex] = new ProxyShopItem
                { Id = itemId, Quantity = item.Quantity, Value = item.Value, Serial = item.SerialNumber, Price = item.Price };
        }
