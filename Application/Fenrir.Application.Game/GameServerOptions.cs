...
/// <summary>Bound from the <c>Game</c> configuration section. One process = one shard hosting the disjoint set of maps assigned to <see cref="ShardId" />.</summary>
public sealed class GameServerOptions
{
    public int Port { get; init; } = 1100;

    /// <summary>Identifies this process's row in <c>runtime.GameServerDirectory</c> and the key looked up in <c>admin.ShardMapAssignments</c>.</summary>
    public byte ShardId { get; init; } = 1;

    /// <summary>Dev-only NTFS junction onto the legacy DATA tree (not committed -- multi-hundred-MB external asset). Resolved against cwd, not <c>AppContext.BaseDirectory</c>.</summary>
    ...
    /// <summary>Anti-speed-hack budget. No legacy source documents an exact speed for M1's map, so this is a generous placeholder, not real game-balance tuning.</summary>
