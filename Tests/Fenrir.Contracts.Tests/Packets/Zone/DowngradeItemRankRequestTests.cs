...
/// <summary>CZ_LOW_ITEM_SEND (CLIENT.h:264, 20-byte payload) — same typedef as <see cref="EnchantItemRequest" />.</summary>
public class CzLowItemSendTests
{
    [Fact]
    public void PayloadSize_MatchesContract()
    {
        Assert.Equal(20, DowngradeItemRankRequest.PayloadSize);
        Assert.Equal(Opcodes.Zone.Incoming.DowngradeItemRank, DowngradeItemRankRequest.Opcode);
    }
    ...
    public void TryRead_GoldenBytes_DecodesEveryField()
    {
        ...
        Assert.True(DowngradeItemRankRequest.TryRead(golden, out var decoded));
    ...
    {
        Assert.False(DowngradeItemRankRequest.TryRead(new byte[19], out _));
    }
}
