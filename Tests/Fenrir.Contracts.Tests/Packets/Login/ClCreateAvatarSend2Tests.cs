...
public class ClCreateAvatarSend2Tests
{
    ...
    public void PayloadSize_MatchesContractConstant()
    {
        ...
        Assert.Equal(41, ClCreateAvatarSend2.PayloadSize);
    }

    [Fact]
    public void RoundTrip_PreservesAllFields()
    {
        var buffer = new byte[ClCreateAvatarSend2.PayloadSize];
        ...
        Assert.True(ClCreateAvatarSend2.TryRead(buffer, out var packet));
    ...
    {
        var buffer = new byte[ClCreateAvatarSend2.PayloadSize - 1];

        Assert.False(ClCreateAvatarSend2.TryRead(buffer, out _));
    }
