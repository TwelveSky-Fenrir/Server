...
/// <summary>CZ_HIGH_ITEM_SEND (CLIENT.h:264, 20-byte payload) — same typedef as <see cref="CzImproveItemSend" />.</summary>
public class CzHighItemSendTests
{
    [Fact]
    public void PayloadSize_MatchesContract()
    {
        Assert.Equal(20, CzHighItemSend.PayloadSize);
        Assert.Equal(Opcodes.Zone.Incoming.HighItemSend, CzHighItemSend.Opcode);
    }
    ...
    public void TryRead_GoldenBytes_DecodesEveryField()
    {
        ...
        Assert.True(CzHighItemSend.TryRead(golden, out var decoded));
    ...
    {
        Assert.False(CzHighItemSend.TryRead(new byte[19], out _));
    }
}
