...
public class ZcUseInventoryItemRecvTests
{
    ...
    {
        Assert.Equal(20, ZcUseInventoryItemRecv.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.UseInventoryItemRecv, ZcUseInventoryItemRecv.Opcode);
    }

    [Fact]
    public void Write_MatchesGoldenBytes()
    {
        var value = new ZcUseInventoryItemRecv { Result = 11, Page = 22, Index = 33, Value = 44, Value2 = 55 };

        var actual = new byte[ZcUseInventoryItemRecv.PayloadSize];
        value.Write(actual);
