...
/// <summary>Outcome of <see cref="IAutoHuntToggleService.ToggleAsync" />: <see cref="Aborted" /> means the caller must disconnect the session.</summary>
public readonly record struct AutoHuntToggleResult(bool Aborted, bool Enabled);

public interface IAutoHuntToggleService
{
    ValueTask<AutoHuntToggleResult> ToggleAsync(int characterId, Zone zone, PlayerRuntimeState state,
