...
public class ZcTribeWorkRecvTests
{
    ...
    {
        Assert.Equal(108, ZcTribeWorkRecv.PayloadSize);
        Assert.Equal(4 + 4 + 100, ZcTribeWorkRecv.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.TribeWorkRecv, ZcTribeWorkRecv.Opcode);
    }
    ...
    public void Write_ProducesGoldenBytes()
    {
        ...
        var packet = new ZcTribeWorkRecv { Result = 0, Sort = 7, Data = data };

        Span<byte> buffer = new byte[ZcTribeWorkRecv.PayloadSize];
        var written = packet.Write(buffer);

        Assert.Equal(ZcTribeWorkRecv.PayloadSize, written);
    ...
    public void Write_EchoesClientDataVerbatim()
    {
        ...
        var packet = new ZcTribeWorkRecv { Result = 0, Sort = 2, Data = echoed };
