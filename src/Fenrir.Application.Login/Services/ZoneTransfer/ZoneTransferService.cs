    ...
    ILogger<ZoneTransferService> logger) : IZoneTransferService
{
        ...
        Guid sessionToken, short accountGrade, IPAddress? sourceAddress, CancellationToken cancellationToken)
    {
        ...
        var ticketCreated = await tickets.CreateAsync(accountId, summary.CharacterId, shard.ShardId,
        ...
        {
            logger.LogWarning(
                "Zone transfer rejected: durable handoff ticket creation failed for account {AccountId} character {CharacterId}",
                accountId, summary.CharacterId);
            return new ZoneTransferResult(ZoneTransferOutcome.ShardUnavailable, shard.Host, zonePort, healedMapId);
        }

        logger.LogInformation(
            "Zone transfer ticket minted: account {AccountId} character {CharacterId} -> zone {MapId} at {Host}:{Port} (shard {ShardId}, ttl {TicketTtlSeconds}s)",
