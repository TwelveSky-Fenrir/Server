...
/// </remarks>
public sealed class CreateAvatarHandler(ICharacterRepository characters, IStarterKitRepository starterKits)
    : IAsyncPacketHandler<CreateAvatarRequest>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var tribe = (byte)packet.Tribe;
        var previousTribe = (byte)packet.PreviousTribe;
        var mapId = SpawnMapIdByTribe[tribe];

        var kit = await starterKits.GetByPreviousTribeAsync(previousTribe, mapId, cancellationToken);

        if (!TryResolveWeaponItemId(kit.Equipment, packet.Weapon, out var weaponItemId))
        {
            loginSession.Abort(DisconnectReason.Malformed);
            return;
        }
        ...
        try
        {
            ...
            session.Send(new CreateAvatarResponse { Result = 0, AvatarInfo = avatarInfo });
        }
        catch (Exception)
        {
