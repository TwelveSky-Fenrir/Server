...
/// <summary>Outcome of one <see cref="ITribeBankService" /> call: whether to abort, and (if not) the wire payload to echo back.</summary>
...
{
    ValueTask<TribeBankResult> ViewAsync(PlayerRuntimeState state, CancellationToken ct);

    ValueTask<TribeBankResult> WithdrawAsync(int slotValue, PlayerRuntimeState state, int characterId,
        CancellationToken ct);

    ValueTask<TribeBankResult> DepositAsync(int slotValue, PlayerRuntimeState state, int characterId,
        CancellationToken ct);
}
