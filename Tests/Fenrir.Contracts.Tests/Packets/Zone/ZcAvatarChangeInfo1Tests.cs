...
public class ZcAvatarChangeInfo1Tests
{
    ...
    {
        Assert.Equal(24, ZcAvatarChangeInfo1.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.AvatarChangeInfo1, ZcAvatarChangeInfo1.Opcode);
    }

    [Fact]
    public void Write_RoundTrips_ViaManualOffsetRead()
    {
        var packet = new ZcAvatarChangeInfo1
        {
            ServerIndex = 5,
        ...
        Span<byte> buffer = new byte[ZcAvatarChangeInfo1.PayloadSize];
        var written = packet.Write(buffer);

        Assert.Equal(ZcAvatarChangeInfo1.PayloadSize, written);
