...
/// </summary>
public sealed class ClChangeMousePasswordSendHandler(IAccountPinRepository pins)
    : IAsyncPacketHandler<ClChangeMousePasswordSend>
{
    ...
    public async ValueTask HandleAsync(ClChangeMousePasswordSend packet, IPacketSession session,
        CancellationToken cancellationToken)
    {
        ...
        {
            session.Send(new LcChangeMousePasswordRecv { Result = 1, MousePassword = ZeroPin });
        ...
        catch (Exception)
        {
            ...
            session.Send(new LcChangeMousePasswordRecv { Result = 2, MousePassword = ZeroPin });
            return;
        }
        ...
        session.Send(new LcChangeMousePasswordRecv { Result = 0, MousePassword = packet.ChangeMousePassword });
    }
}
