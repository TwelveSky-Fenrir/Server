...
///         coordinate triples -- slot 1: (-232, 36, 2), slot 2: (232, 36, 2), slot 2 mirroring slot 1 by
///         negating only the first component -- unless it is itself mid-connection/mid-zone-transfer or
///         already engaged in another duel-related state -- see <see cref="GmCallPvpPayload" />'s own remarks
///         for the matching semantics, and this interface's implementation for the one remaining, permanently
///         unrecoverable magnitude (the identity of a per-character cooldown field the relocation incidentally
///         resets -- see that type's own remarks for why an exhaustive second legacy-research pass still could
///         not resolve it).
///     </para>
