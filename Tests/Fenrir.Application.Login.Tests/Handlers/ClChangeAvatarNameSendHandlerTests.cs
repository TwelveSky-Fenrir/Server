...
public class ClChangeAvatarNameSendHandlerTests
{
    ...
    public async Task HandleAsync_RenameSucceeds_RepliesResultZero()
    {
        ...
        var handler = new ClChangeAvatarNameSendHandler(renames);
        var (session, pipe) = CreateSessionInCharSelect();

        await handler.HandleAsync(
            new ClChangeAvatarNameSend { AvatarPost = 1, ChangeAvatarName = "NewName", Page = 0, Index = 5 },
        ...
        await PacketAssert.AssertSentAsync(pipe, new LcChangeAvatarNameRecv { Result = 0 });
    ...
    public async Task HandleAsync_ProcReturnsLegacyFailureCode_ForwardsItVerbatim(int code)
    {
        ...
        await handler.HandleAsync(
            new ClChangeAvatarNameSend { AvatarPost = 0, ChangeAvatarName = "Name", Page = 0, Index = 0 }, session,
            CancellationToken.None);

        await PacketAssert.AssertSentAsync(pipe, new LcChangeAvatarNameRecv { Result = code });
    }
    ...
    public async Task HandleAsync_RepositoryThrows_RepliesResult101WithoutDisconnecting()
    {
        ...
        await PacketAssert.AssertSentAsync(pipe, new LcChangeAvatarNameRecv { Result = 101 });
        Assert.Null(session.DisconnectReason);
    }
        ...
        int page, int index)
    {
        ...
        await handler.HandleAsync(
            new ClChangeAvatarNameSend { AvatarPost = avatarPost, ChangeAvatarName = name, Page = page, Index = index },
            session, CancellationToken.None);
