...
public class ZcProcessAttackRecvTests
{
    ...
    {
        Assert.Equal(68, AttackResponse.PayloadSize);
        Assert.Equal(AttackForProtocol.WireSize, AttackResponse.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.Attack, AttackResponse.Opcode);
    }
    ...
    public void Write_MatchesAttackForProtocolWrite()
    {
        ...
        Span<byte> buffer = new byte[AttackResponse.PayloadSize];
        var written = attack.Write(buffer);

        Assert.Equal(AttackForProtocol.WireSize, written);

        var packet = new AttackResponse { AttackInfo = attack };
        Span<byte> writeBuffer = new byte[AttackResponse.PayloadSize];
    ...
    public void Write_ProducesGoldenBytes()
    {
        ...
        var golden = new byte[AttackResponse.PayloadSize];
