...
public class ZcCostumeStateRecvTests
{
    ...
    {
        Assert.Equal(32, ZcCostumeStateRecv.PayloadSize);
        Assert.Equal(8 * 4, ZcCostumeStateRecv.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.CostumeStateRecv, ZcCostumeStateRecv.Opcode);
    }

    [Fact]
    public void Write_ProducesGoldenBytes()
    {
        var packet = new ZcCostumeStateRecv
        {
            Result = 0,
            Sort = 5,
            Value = 3,
        ...
        var golden = new byte[ZcCostumeStateRecv.PayloadSize];
        ...
        Span<byte> buffer = new byte[ZcCostumeStateRecv.PayloadSize];
        var written = packet.Write(buffer);

        Assert.Equal(ZcCostumeStateRecv.PayloadSize, written);
