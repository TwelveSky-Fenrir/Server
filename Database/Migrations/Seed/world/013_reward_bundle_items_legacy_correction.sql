-- Corrects 012_reward_bundles.sql's placeholder seed (ItemId=12 for all 7 slots, flagged there as
-- "LOW-CONFIDENCE ... signs of an abandoned feature" because the legacy `rewardinfo` MySQL table the
-- placeholder was meant to approximate is itself unreliable/near-empty in the dump).
--
-- This replaces those placeholder rows with the one concrete, cited, confirmed-read reward table that
-- legacy actually ever sends to a client: `ts25extra`'s ZONE_GET_REWARD_ITEM_FOR_EXTRA_SEND handler reads
-- `rewardinfo` via MyDB::GetRewardItem, then unconditionally DISCARDS that result and overwrites both the
-- result flag and all 7 item slots with a fixed, hardcoded table before responding
-- (Server/ts25extra/S04_MyWork02.cpp:1352-1372, confirmed read -- see the fully-cited
...
-- (080_items.sql) under names matching the source's own inline day-labeling comments, which independently
...
-- decision, not settled by this script. This script only replaces a lower-confidence, uncited placeholder
-- with the sole cited, confirmed-read value set available -- it does not touch the surrounding
-- data-store-backed architecture (world.RewardBundles / world.RewardBundleItems / IWorldDataRepository),
-- which is Fenrir's own, already-shipped, idiomatic reimplementation choice and is left exactly as-is.
DELETE
FROM world.RewardBundleItems
WHERE RewardBundleId = 1;
