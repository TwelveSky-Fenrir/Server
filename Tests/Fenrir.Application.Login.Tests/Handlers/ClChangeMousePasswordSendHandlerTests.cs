...
public class ClChangeMousePasswordSendHandlerTests
{
    ...
    public async Task HandleAsync_CorrectCurrentPin_StoresNewPinAndOpensCharSelect()
    {
        ...
        var handler = new ClChangeMousePasswordSendHandler(pins);
        var (session, pipe) = CreateSessionInPinRequired();

        await handler.HandleAsync(
            new ClChangeMousePasswordSend { MousePassword = "1111", ChangeMousePassword = "2222" }, session,
            CancellationToken.None);

        Assert.Equal(LoginSessionState.CharSelect, session.State);
        Assert.Null(session.DisconnectReason);
        await PacketAssert.AssertSentAsync(pipe, new LcChangeMousePasswordRecv { Result = 0, MousePassword = "2222" });
    }
    ...
    public async Task HandleAsync_InvalidNewFormat_AbortsAsMalformed()
    {
        ...
        await handler.HandleAsync(
            new ClChangeMousePasswordSend { MousePassword = "1111", ChangeMousePassword = "22x2" }, session,
            CancellationToken.None);
    ...
    public async Task HandleAsync_WrongCurrentPin_RepliesResultOneAndStrikes_ThirdDisconnects()
    {
        ...
        var handler = new ClChangeMousePasswordSendHandler(pins);
        var (session, pipe) = CreateSessionInPinRequired();
        var attempt = new ClChangeMousePasswordSend { MousePassword = "0000", ChangeMousePassword = "2222" };

        await handler.HandleAsync(attempt, session, CancellationToken.None);
        await PacketAssert.AssertSentAsync(pipe, new LcChangeMousePasswordRecv { Result = 1, MousePassword = "0000" });
        Assert.Null(session.DisconnectReason);
    ...
    public async Task HandleAsync_StorageFailure_RepliesResultTwoWithoutDisconnecting()
    {
        ...
        await PacketAssert.AssertSentAsync(pipe, new LcChangeMousePasswordRecv { Result = 2, MousePassword = "0000" });
        Assert.Null(session.DisconnectReason);
