...
public class ZcSetDeputyPshopRecvTests
{
    ...
    {
        Assert.Equal(876, ZcSetDeputyPshopRecv.PayloadSize);
        Assert.Equal(4 + ProxyShopUserInfo.WireSize + 4 + 4 + 36 + 4, ZcSetDeputyPshopRecv.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.SetDeputyPshopRecv, ZcSetDeputyPshopRecv.Opcode);
    }
    ...
    public void Write_RoundTrips_ViaManualOffsetRead()
    {
        ...
        var packet = new ZcSetDeputyPshopRecv
        {
            Result = 0,
            ProxyUser = proxyUser,
            Page = 2,
        ...
        Span<byte> buffer = new byte[ZcSetDeputyPshopRecv.PayloadSize];
        var written = packet.Write(buffer);

        Assert.Equal(ZcSetDeputyPshopRecv.PayloadSize, written);
    ...
    public void Write_ProducesGoldenBytes()
    {
        ...
        var golden = new byte[ZcSetDeputyPshopRecv.PayloadSize];
