...
public class CzBuyPshopSendTests
{
    ...
    {
        Assert.Equal(45, CzBuyPshopSend.PayloadSize);
        Assert.Equal(4 + 13 + 7 * 4, CzBuyPshopSend.PayloadSize);
        Assert.Equal(Opcodes.Zone.Incoming.BuyPshopSend, CzBuyPshopSend.Opcode);
    }

    [Fact]
    public void TryRead_DecodesFieldsFromManuallyEncodedBuffer()
    {
        Span<byte> buffer = stackalloc byte[CzBuyPshopSend.PayloadSize];
        ...
        var ok = CzBuyPshopSend.TryRead(buffer, out var packet);
