...
public class ZcItemActionRecvTests
{
    ...
    {
        Assert.Equal(96, GroundItemReplicationResponse.PayloadSize);
        Assert.Equal(4 + 4 + ObjectForItem.WireSize + 4, GroundItemReplicationResponse.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.GroundItemReplication, GroundItemReplicationResponse.Opcode);
    }
    ...
    public void Write_RoundTrips_ViaManualOffsetRead()
    {
        ...
        var packet = new GroundItemReplicationResponse
        {
            ServerIndex = 8,
        ...
        Span<byte> buffer = new byte[GroundItemReplicationResponse.PayloadSize];
        var written = packet.Write(buffer);

        Assert.Equal(GroundItemReplicationResponse.PayloadSize, written);
    ...
    public void Write_ProducesGoldenBytes_WithInternalPaddingZeroed()
    {
        ...
        var golden = new byte[GroundItemReplicationResponse.PayloadSize];
