...
/// <summary>CZ_ADD_ITEM_SEND (CLIENT.h:264, 20-byte payload) — same typedef as <see cref="CzImproveItemSend" />.</summary>
public class CzAddItemSendTests
{
    [Fact]
    public void PayloadSize_MatchesContract()
    {
        Assert.Equal(20, CzAddItemSend.PayloadSize);
        Assert.Equal(Opcodes.Zone.Incoming.AddItemSend, CzAddItemSend.Opcode);
    }
    ...
    public void TryRead_GoldenBytes_DecodesEveryField()
    {
        ...
        Assert.True(CzAddItemSend.TryRead(golden, out var decoded));
    ...
    {
        Assert.False(CzAddItemSend.TryRead(new byte[19], out _));
    }
}
