...
/// </remarks>
public sealed class SearchShopListingsHandler(WorldDataCache worldData)
    : IInlinePacketHandler<SearchShopListingsRequest>
{
    ...
    public void Handle(in SearchShopListingsRequest packet, IPacketSession session)
    {
        ...
        foreach (var seller in zone.Players)
        {
            ...
            for (var page = 0; page < PshopPurchasePolicy.MaxPages && sent < MaxResults; page++)
            for (var slot = 0; slot < PshopPurchasePolicy.MaxSlots && sent < MaxResults; slot++)
            {
                ...
                session.Send(new SearchShopListingsResponse
                {
