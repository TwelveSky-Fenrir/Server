...
/// <summary>
///     Covers <see cref="PetSteppedAttackPowerCategoryTable" /> against <c>PETSYSTEM::ReturnAttackPower2</c>'s
///     own item-id -&gt; category table (<c>GameSystem_07_Pet.cpp:1722-1802</c>) -- the B8-pet-growth-depth
///     contract's Part B, including the deliberate discrepancy with <see cref="DoublePetExpTimerGate" />'s
///     own, larger table.
/// </summary>
public class PetSteppedAttackPowerCategoryTableTests
{
    ...
    [InlineData(8202, 40_000_000)] // GIFT_EVENT category 0
    [InlineData(1006, 80_000_000)]
    [InlineData(17052, 80_000_000)]
    [InlineData(8211, 80_000_000)] // GIFT_EVENT category 1
    [InlineData(1012, 160_000_000)]
    [InlineData(17053, 160_000_000)]
    [InlineData(8212, 160_000_000)] // GIFT_EVENT category 2
    [InlineData(1016, 320_000_000)]
    [InlineData(2160, 320_000_000)]
    [InlineData(17057, 320_000_000)]
    [InlineData(8216, 320_000_000)] // GIFT_EVENT category 3
    ...
    // These ids ARE members of DoublePetExpTimerGate's own table but are wholly absent from this
    // (strictly smaller) six-tier attack-power table -- the contract's own re-verified discrepancy: a pet
    // built from any of these can reach and be frozen at 200% growth by the timer gate while permanently
    // receiving a flatly-zero six-tier attack bonus.
    [Theory]
    [InlineData(541)]
    [InlineData(542)]
    [InlineData(547)]
