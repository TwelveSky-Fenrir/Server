...
///     (<see cref="ClLoginMousePasswordSendHandler" />'s <c>MaxPinFailures</c>).
/// </summary>
public class ClLoginMousePasswordSendHandlerTests
{
    ...
    public async Task HandleAsync_CorrectPin_OpensCharSelect()
    {
        ...
        var handler = new ClLoginMousePasswordSendHandler(pins);
        var (session, pipe) = CreateSessionInPinRequired();

        await handler.HandleAsync(new ClLoginMousePasswordSend { MousePasswordInput = "4242" }, session,
            CancellationToken.None);

        Assert.Equal(LoginSessionState.CharSelect, session.State);
        Assert.Null(session.DisconnectReason);
        await PacketAssert.AssertSentAsync(pipe, new LcLoginMousePasswordRecv { Result = 0 });
    }
    ...
    public async Task HandleAsync_InvalidFormat_AbortsAsMalformed()
    {
        ...
        await handler.HandleAsync(new ClLoginMousePasswordSend { MousePasswordInput = "42x2" }, session,
            CancellationToken.None);
    ...
    public async Task HandleAsync_WrongPin_TwiceThenThird_StaysUntilThirdStrikeDisconnects()
    {
        ...
        await handler.HandleAsync(new ClLoginMousePasswordSend { MousePasswordInput = "0000" }, session,
            CancellationToken.None);
        await PacketAssert.AssertSentAsync(pipe, new LcLoginMousePasswordRecv { Result = 1 });
        Assert.Null(session.DisconnectReason);
