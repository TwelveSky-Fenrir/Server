...
public class CzSetDeputyPshopSendTests
{
    ...
    {
        Assert.Equal(73, CzSetDeputyPshopSend.PayloadSize);
        Assert.Equal(13 + 12 * 4 + 12, CzSetDeputyPshopSend.PayloadSize);
        Assert.Equal(Opcodes.Zone.Incoming.SetDeputyPshopSend, CzSetDeputyPshopSend.Opcode);
    }

    [Fact]
    public void TryRead_DecodesFieldsFromManuallyEncodedBuffer()
    {
        Span<byte> buffer = stackalloc byte[CzSetDeputyPshopSend.PayloadSize];
        ...
        var ok = CzSetDeputyPshopSend.TryRead(buffer, out var packet);
