...
public class ZcBuyPshopRecvTests
{
    ...
    {
        Assert.Equal(52, ZcBuyPshopRecv.PayloadSize);
        Assert.Equal(4 + 4 + 4 + 4 + 24 + 12, ZcBuyPshopRecv.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.BuyPshopRecv, ZcBuyPshopRecv.Opcode);
    }
    ...
    public void Write_ProducesGoldenBytes()
    {
        ...
        var actual = new byte[ZcBuyPshopRecv.PayloadSize];
        var written = packet.Write(actual);
        Assert.Equal(ZcBuyPshopRecv.PayloadSize, written);

        var expected = new byte[ZcBuyPshopRecv.PayloadSize];
