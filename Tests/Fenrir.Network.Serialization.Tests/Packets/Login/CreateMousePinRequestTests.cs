...
public class ClCreateMousePasswordSendTests
{
    ...
    public void PayloadSize_MatchesContractConstant()
    {
        ...
        Assert.Equal(5, CreateMousePinRequest.PayloadSize);
    }

    [Fact]
    public void RoundTrip_PreservesAllFields()
    {
        var buffer = new byte[CreateMousePinRequest.PayloadSize];
        WriteFixedString(buffer.AsSpan(0, 5), "1234");

        Assert.True(CreateMousePinRequest.TryRead(buffer, out var packet));
    ...
    {
        var buffer = new byte[CreateMousePinRequest.PayloadSize - 1];

        Assert.False(CreateMousePinRequest.TryRead(buffer, out _));
    }
