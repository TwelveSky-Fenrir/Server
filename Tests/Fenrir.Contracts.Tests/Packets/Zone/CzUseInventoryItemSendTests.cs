...
public class CzUseInventoryItemSendTests
{
    ...
    {
        Assert.Equal(12, CzUseInventoryItemSend.PayloadSize);
        Assert.Equal(Opcodes.Zone.Incoming.UseInventoryItemSend, CzUseInventoryItemSend.Opcode);
    }
    ...
    public void TryRead_GoldenBytes_DecodesEveryField()
    {
        ...
        Assert.True(CzUseInventoryItemSend.TryRead(golden, out var decoded));
    ...
    {
        Assert.False(CzUseInventoryItemSend.TryRead(new byte[11], out _));
    }
}
