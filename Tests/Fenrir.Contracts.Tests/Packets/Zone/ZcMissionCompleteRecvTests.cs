...
public class ZcMissionCompleteRecvTests
{
    ...
    {
        Assert.Equal(24, ZcMissionCompleteRecv.PayloadSize);
        Assert.Equal(4 + 4 + MissionDate.WireSize, ZcMissionCompleteRecv.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.MissionCompleteRecv, ZcMissionCompleteRecv.Opcode);
    }
    ...
    private static ZcMissionCompleteRecv CreatePopulated()
    {
        return new ZcMissionCompleteRecv
        {
            Sort = 2,
            Result = 0,
    ...
    private static void EncodeGolden(Span<byte> destination, ZcMissionCompleteRecv value)
    {
