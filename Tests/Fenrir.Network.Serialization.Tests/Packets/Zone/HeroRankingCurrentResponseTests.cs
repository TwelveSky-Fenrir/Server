...
public class ZcLastHeroRankInfoRecvTests
{
    ...
    {
        Assert.Equal(684, HeroRankingCurrentResponse.PayloadSize);
        Assert.Equal(4 + HeroRank.WireSize, HeroRankingCurrentResponse.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.HeroRankingCurrent, HeroRankingCurrentResponse.Opcode);
    }
    ...
    private static HeroRankingCurrentResponse CreatePopulated()
    {
        var v = new SequentialValueFactory();
        return new HeroRankingCurrentResponse
        {
            Result = v.NextInt(),
    ...
    private static void EncodeGolden(Span<byte> destination, HeroRankingCurrentResponse value)
    {
