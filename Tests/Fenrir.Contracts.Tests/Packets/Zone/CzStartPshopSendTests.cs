...
public class CzStartPshopSendTests
{
    ...
    {
        Assert.Equal(1236, CzStartPshopSend.PayloadSize);
        Assert.Equal(4 + PshopInfo.WireSize, CzStartPshopSend.PayloadSize);
        Assert.Equal(Opcodes.Zone.Incoming.StartPshopSend, CzStartPshopSend.Opcode);
    }
    ...
    public void TryRead_GoldenBytes_DecodesEveryField()
    {
        ...
        var golden = new byte[CzStartPshopSend.PayloadSize];
        ...
        var ok = CzStartPshopSend.TryRead(golden, out var packet);
