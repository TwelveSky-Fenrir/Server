...
public class ZcAvatarEffectValueInfoTests
{
    ...
    {
        Assert.Equal(428, ZcAvatarEffectValueInfo.PayloadSize);
        Assert.Equal(4 + 4 + 70 * 4 + 35 * 4, ZcAvatarEffectValueInfo.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.AvatarEffectValueInfo, ZcAvatarEffectValueInfo.Opcode);
    }
    ...
    public void Write_RoundTrips_ViaManualOffsetRead()
    {
        ...
        var packet = new ZcAvatarEffectValueInfo
        {
            ServerIndex = 17,
        ...
        Span<byte> buffer = new byte[ZcAvatarEffectValueInfo.PayloadSize];
        var written = packet.Write(buffer);

        Assert.Equal(ZcAvatarEffectValueInfo.PayloadSize, written);
