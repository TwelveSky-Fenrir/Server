    ...
    BEGIN
        ...
        (
            ...
            FROM runtime.ZoneEventRelayOutbox WITH (UPDLOCK, READPAST, READCOMMITTEDLOCK, ROWLOCK,
                                                     INDEX(IX_ZoneEventRelayOutbox_PublishedRetention))
            WHERE PublishStatus = 2
              AND PublishedAtUtc <= DATEADD(HOUR, -24, @NowUtc)
            ORDER BY PublishedAtUtc, OutboxId
        )
        DELETE FROM ExpiredPublishedEvents;

        SELECT @ActiveCount = COUNT_BIG(*)
        FROM runtime.ZoneEventRelayOutbox WITH (TABLOCKX, HOLDLOCK)
        ...
        BEGIN
            INSERT INTO runtime.ZoneEventRelayOutbox
                (AuthenticatedSource, SourceShardId, Sort, Data, OperationId, CorrelationId, PublishStatus,
                 ...
                 CreatedAtUtc)
            VALUES
                (@AuthenticatedSource, @SourceShardId, @Sort, @Data, @OperationId, @CorrelationId, 0, 0,
