...
public enum SkillValueKind
{
    ...
    /// <summary>2: <c>RecoverInfo1</c> -- meditation HP-regen-per-tick DIVISOR (report 05 §7 pt.3) OR a targeted heal's flat HP amount (skills 106/108/110) -- two different interpretations of the SAME column, both verified at their own call sites; never conflate them.</summary>
    RecoverInfo1 = 2,

    /// <summary>3: <c>RecoverInfo2</c> -- meditation MP-regen-per-tick divisor, or a targeted heal's flat MP amount (skills 107/109/111) -- same dual-interpretation caveat as <see cref="RecoverInfo1" />.</summary>
    RecoverInfo2 = 3,
    ...
    /// <summary>7: <c>AttackInfo1</c> -- the attack-power-up RATIO (percent) fed into <c>CalDamage</c> for a skill-based attack (report 05 §4).</summary>
    AttackPowerRatio = 7,
    ...
    /// <summary>10: <c>RunTime</c> -- buff DURATION in legacy ticks (before the (unmodeled) <c>mSupportSkillTimeUpRatio</c> multiplier -- report 12 §4.2).</summary>
    RunTime = 10,
    ...
    /// <summary>20: <c>ShieldLifeUp</c> -- writes buff slot 9 (Holy Shield, skills 82/79), value = ratio% × caster MaxLife × 0.01.</summary>
    ShieldLifeUp = 20,
