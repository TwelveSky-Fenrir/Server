...
public class ZcRuneSystemRecvTests
{
    ...
    {
        Assert.Equal(20, ZcRuneSystemRecv.PayloadSize);
        Assert.Equal(Opcodes.Zone.Outgoing.RuneSystemRecv, ZcRuneSystemRecv.Opcode);
    }

    [Fact]
    public void Write_MatchesGoldenBytes()
    {
        var value = new ZcRuneSystemRecv { Result = 11, Page = 22, Index = 33, ItemIndex = 44, RuneIndex = 55 };

        var actual = new byte[ZcRuneSystemRecv.PayloadSize];
        value.Write(actual);
