using System.Text;
using Fenrir.Contracts.Packets.Login;

namespace Fenrir.Contracts.Tests.Packets.Login;

public class ClChangeMousePasswordSendTests
{
    [Fact]
    public void PayloadSize_MatchesContractConstant()
    {
        // ExpectedSize=19 (9-byte inbound header) -> 10-byte payload (2x char[5]).
        Assert.Equal(10, ClChangeMousePasswordSend.PayloadSize);
    }

    [Fact]
    public void RoundTrip_PreservesAllFields()
    {
        var buffer = new byte[ClChangeMousePasswordSend.PayloadSize];
        WriteFixedString(buffer.AsSpan(0, 5), "1234");
        WriteFixedString(buffer.AsSpan(5, 5), "5678");

        Assert.True(ClChangeMousePasswordSend.TryRead(buffer, out var packet));

        Assert.Equal("1234", packet.MousePassword);
        Assert.Equal("5678", packet.ChangeMousePassword);
    }

    [Fact]
    public void TryRead_BufferTooShort_Fails()
    {
        var buffer = new byte[ClChangeMousePasswordSend.PayloadSize - 1];

        Assert.False(ClChangeMousePasswordSend.TryRead(buffer, out _));
    }

    private static void WriteFixedString(Span<byte> destination, string value)
    {
        destination.Clear();
        Encoding.Latin1.GetBytes(value, destination);
    }
}
