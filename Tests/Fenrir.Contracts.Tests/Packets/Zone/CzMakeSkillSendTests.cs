...
/// <summary>CZ_MAKE_SKILL_SEND (CLIENT.h:289, 36-byte payload) — same typedef as <see cref="CzMakeItemSend" />.</summary>
public class CzMakeSkillSendTests
{
    [Fact]
    public void PayloadSize_MatchesContract()
    {
        Assert.Equal(36, CzMakeSkillSend.PayloadSize);
        Assert.Equal(Opcodes.Zone.Incoming.MakeSkillSend, CzMakeSkillSend.Opcode);
    }
    ...
    public void TryRead_GoldenBytes_DecodesEveryField()
    {
        ...
        Assert.True(CzMakeSkillSend.TryRead(golden, out var decoded));
    ...
    {
        Assert.False(CzMakeSkillSend.TryRead(new byte[35], out _));
    }

    private static CzMakeSkillSend CreatePopulated()
    {
        return new CzMakeSkillSend
        {
            Sort = 11,
            Page1 = 22,
            Index1 = 33,
    ...
    private static void EncodeGolden(Span<byte> destination, CzMakeSkillSend value)
    {
