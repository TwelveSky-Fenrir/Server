...
public class ClDeleteAvatarSendTests
{
    ...
    public void PayloadSize_MatchesContractConstant()
    {
        ...
        Assert.Equal(12, ClDeleteAvatarSend.PayloadSize);
    }

    [Fact]
    public void RoundTrip_PreservesAllFields()
    {
        var buffer = new byte[ClDeleteAvatarSend.PayloadSize];
        ...
        Assert.True(ClDeleteAvatarSend.TryRead(buffer, out var packet));
    ...
    {
        var buffer = new byte[ClDeleteAvatarSend.PayloadSize - 1];

        Assert.False(ClDeleteAvatarSend.TryRead(buffer, out _));
    }
}
