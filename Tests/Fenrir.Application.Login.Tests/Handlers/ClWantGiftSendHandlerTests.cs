...
public class ClWantGiftSendHandlerTests
{
    ...
    {
        var handler = new ClWantGiftSendHandler();
        var (session, pipe) = CreateSessionInCharSelect();

        handler.Handle(new ClWantGiftSend { Sort = 0, GiftInfoIndex = index }, session);

        Assert.Null(session.DisconnectReason);
        await PacketAssert.AssertSentAsync(pipe, new LcWantGiftRecv { Result = 1 });
    }
