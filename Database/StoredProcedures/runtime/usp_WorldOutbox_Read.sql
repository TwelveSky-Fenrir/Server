...
BEGIN
    ...
    SELECT OutboxId, AuthenticatedSource, SourceShardId, SourceSequence, DestinationShardId, PayloadCategory, Payload,
           PayloadHash, CorrelationId, IdempotencyKey, AttemptCount
    ...
    BEGIN
        ...
        (
            ...
            FROM runtime.WorldOutbox WITH (UPDLOCK, READPAST, READCOMMITTEDLOCK, ROWLOCK,
                                           INDEX(IX_WorldOutbox_Claim))
            WHERE DestinationShardId = @DestinationShardId
              AND
              (
